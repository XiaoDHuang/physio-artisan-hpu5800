# voice-asr-api Specification

## Purpose

提供 `POST /api/asr` 语音转文字端点，经千问 ASR 适配器转写浏览器录音。

## Requirements

### Requirement: 语音转文字端点 /api/asr

系统 SHALL 提供 `POST /api/asr`（`multipart/form-data`：`audio` 文件 + `format` 字符串），收录音并返回转写文本。响应 200 SHALL 为 `{ "text": string, "duration_ms"?: number, "model": string }`；前端按此结构处理。
...
系统 SHALL 用适配器 `transcribe_audio(data, mime, filename)` 封装 ASR 调用，默认经现有 OpenAI 兼容网关调用千问语音模型，路径由 `ASR_MODE` 配置切换（`transcriptions` 或 `chat_audio` 多模态 base64）。模型与网关 SHALL 可配（`ASR_MODEL` / `ASR_BASE_URL` / `ASR_API_KEY`）。
