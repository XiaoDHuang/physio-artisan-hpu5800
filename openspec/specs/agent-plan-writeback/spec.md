# agent-plan-writeback Specification

## Purpose

将 `/plan`（多智能体报告）产出经 adapter 映射为 v3.1 `user_plans` 计划形并幂等落库，使三页（运动/睡眠/饮食）建议与训练方案读到本次 AI 决策。与第三层换源（`add-three-pages-real-data`）配合使用：三页取数层已改为优先读 `user_plans`（缺失回退 seed）。
